...
- 🔒 **Private Responses**: All loan information is private (only visible to you)
...
- `/lend member:<@user> amount:<amount> [description:<text>]`
  ...
  - **Note**: If you already have an unpaid loan with this person, the new amount will be added to the existing loan instead of creating a separate one
...
- `/setcurrency`
  ...
  - The modal has two fields:
    ...
    - **Suffix**: Text after the amount (e.g. `ISK`, `USD`, `coins`). A space is automatically added before the suffix.
...
The bot uses SQLite with Tortoise ORM for data persistence. The database file (`db.sqlite3`) is automatically created when you first run the bot.
...
If you modify the models, Tortoise ORM will automatically update the schema on next run. For production, consider using Aerich for proper migrations.
